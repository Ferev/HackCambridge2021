...
class FawkesMaskGeneration:
    ...
    def __init__(self, sess, bottleneck_model_ls, mimic_img=MIMIC_IMG,
        ...
        self.assign_modifier = tf.compat.v1.placeholder(tf.float32, self.input_shape)
        if self.MIMIC_IMG:
            self.assign_timg_tanh = tf.compat.v1.placeholder(
                tf.float32, self.input_shape)
        else:
            self.assign_bottleneck_t_raw = tf.compat.v1.placeholder(
                tf.float32, self.bottleneck_shape)
        self.assign_simg_tanh = tf.compat.v1.placeholder(tf.float32, self.input_shape)
        self.assign_const = tf.compat.v1.placeholder(tf.float32, (batch_size))
        self.assign_mask = tf.compat.v1.placeholder(tf.bool, (batch_size))
        self.assign_weights = tf.compat.v1.placeholder(tf.float32, self.bottleneck_shape)
        ...
        self.learning_rate_holder = tf.compat.v1.placeholder(tf.float32, shape=[])

        optimizer = tf.compat.v1.train.AdadeltaOptimizer(self.learning_rate_holder)
        ...
        self.init = tf.compat.v1.variables_initializer(var_list=[self.modifier] + new_vars)
